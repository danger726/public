...
void FastSinCos( float4 val, out float4 s, out float4 c )
{
	...
	float4 sin7 = {1.0, -0.16161616, 0.0083333, -0.00019841};
...
fixed4 TerrainWaveGrass( inout float4 vertex, float waveAmount, fixed4 color )
{
	...
	color.a = saturate( 2.0 * (_WaveAndDistance.w - dot( offset, offset )) * _CameraPosition.w );
...
inline float4 Squash( in float4 pos )
{
	...
	pos = float4( lerp( projectedVertex, pos.xyz, _SquashAmount ), 1.0 );
...
inline void ExpandBillboard( in float4x4 mat, inout float4 pos, inout float3 normal, inout float4 tangent )
{
	...
	// billboard normal
	float3 norb = normalize( mul( float4( normal, 0.0 ), mat ) ).xyz;

	// billboard tangent
	float3 tanb = normalize( mul( float4( tangent.xyz, 0.0 ), mat ) ).xyz;

	pos += mul( float4( normal.xy, 0.0, 0.0 ), mat ) * isBillboard;
	normal = lerp( normal, norb, isBillboard );
	tangent = lerp( tangent, float4( tanb, -1.0 ), isBillboard );
}
...
inline float4 AnimateVertex( float4 pos, float3 normal, float4 animParams )
{
	...
	float fObjPhase = dot( unity_ObjectToWorld._14_24_34, 1.0 );
